...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/07 11:45:57 by gquence           #+#    #+#             */
/*   Updated: 2019/08/07 13:54:05 by gquence          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			*get_colours(t_cl *cl_dev, t_cl_param *ptr)
{
	...
	dim[0] = WIDTH;
	dim[1] = HEIGHT;
	output = clCreateBuffer(cl_dev->dev_context,
		CL_MEM_WRITE_ONLY, (WIDTH + 32) * HEIGHT * sizeof(int), NULL, NULL);
	input = clCreateBuffer(cl_dev->dev_context,
		CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(t_cl_param), ptr, NULL);
	if (!input || !output)
		return (NULL);
	cl_dev->ret_err = clSetKernelArg(cl_dev->kernel, 0,
		sizeof(cl_mem), &output);
	cl_dev->ret_err |= clSetKernelArg(cl_dev->kernel, 1,
		sizeof(cl_mem), &input);
	cl_dev->ret_err = clEnqueueNDRangeKernel(cl_dev->cmd_queue, cl_dev->kernel,
		2, NULL, dim, NULL, 0, NULL, NULL);
	cl_dev->ret_err = clEnqueueReadBuffer(cl_dev->cmd_queue, output,
		CL_TRUE, 0, (WIDTH + 32) * HEIGHT * sizeof(int), arg, 0, NULL, NULL);
