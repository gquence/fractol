...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/06 17:29:01 by gquence           #+#    #+#             */
/*   Updated: 2019/08/06 17:29:14 by gquence          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     get_deviceattr(t_cl *ptr_dev)
{
    ...
    if (!platform_count)
        return (1);
    platforms = (cl_platform_id *)malloc(sizeof(cl_platform_id) * (int)platform_count);
    clGetPlatformIDs(platform_count, platforms, NULL);
    ret_err = clGetDeviceIDs(platforms[0], CL_DEVICE_TYPE_GPU, 1, &(ptr_dev->device_id), NULL);
    if (ret_err != CL_SUCCESS)
    {
        free(platforms);
        return (2);
    }
    ptr_dev->dev_context = clCreateContext(0, 1, &(ptr_dev->device_id), NULL, NULL, &ret_err);
    if (!ptr_dev->dev_context)
    {
        free(platforms);
        return (3);
    }
...
{
    ptr_dev->cmd_queue = clCreateCommandQueue(ptr_dev->dev_context, ptr_dev->device_id, 0, &ptr_dev->ret_err);
...
int        cl_init(t_cl *cl_dev, char *filename, char *fractol_name)
{
    ...
    if ((ret_err = get_program(cl_dev, filename)))
        return (2);
    cl_dev->ret_err = clBuildProgram(cl_dev->program, 0, NULL, "-I ./", NULL, NULL);
    if (!cl_dev->program)
        return (3);
    cl_dev->kernel = clCreateKernel(cl_dev->program, fractol_name, &cl_dev->ret_err);
